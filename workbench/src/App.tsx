...
// Where Agent mode looks for agent definition .md files, in order.
const AGENT_DIRS = ["/Users/rowe/.claude/agents", "/Users/rowe/BFD/agents"];

// The categorized registry Agent mode's picker reads to enumerate projects
// and agent groups (see AGENTS.md itself for the format: "## Project: X" /
// "## Agent Group: X" headings, each followed by free-text body).
const AGENTS_REGISTRY_PATH = "/Users/rowe/BFD/workbench/AGENTS.md";

interface AgentCategory {
  kind: "Project" | "Agent Group";
  name: string;
  body: string;
}

function parseAgentRegistry(md: string): AgentCategory[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    categories.push({
      kind: m[1] as "Project" | "Agent Group",
      name: m[2].trim(),
...
const STARTUP_TABS_CONFIG = "/Users/rowe/BFD/.bfd/startup_tabs.json";
...
// Sent once the user has picked a category from the enumerated Agent-mode
// picker (or "New"). Scoped to that pick, rather than a generic "list
// everything" kickoff, so the discussion opens already pointed at the right
// project/agent group.
const AGENT_KICKOFF_PROMPT = (category: AgentCategory) => `[System: The user just switched BFD into Agent mode and picked "${category.kind}: ${category.name}" from the enumerated list in AGENTS.md. Greet them briefly and open a discussion scoped to that ${category.kind.toLowerCase()}. Its registry entry:

${category.body}

If matching agent definition files are loaded in your context below, mention the ones relevant to this ${category.kind.toLowerCase()} (name + one-line purpose). Ask whether they want help using, improving, or extending an agent here. Keep it short and conversational.]`;

const NEW_AGENT_KICKOFF_PROMPT = `[System: The user just switched BFD into Agent mode and picked "New" from the enumerated list — they want to set up a project or agent group that isn't in AGENTS.md yet, rather than work within an existing one. Greet them, briefly note what's already in the registry so they know what exists, then ask what the new project or agent group is for and help them scaffold its first agent definition file (and an AGENTS.md entry once it's real). Keep it short and conversational.]`;
...
function toolActivityLine(name: string, input: any): string {
  ...
  if (typeof p === "string") return `${base} ${p.split("/").pop()}`;
...
const BASE_PROMPT = `You are BFD, a local AI workbench assistant. You help with coding, data analysis, and general questions. Be helpful, concise, and practical.

You have access to the local file system via tools. Always use absolute paths starting with /Users/rowe/.
...
Memory files:
- Global: /Users/rowe/bfd/.bfd/global.md
...
// --- App ---
function App() {
  const [activeView, setActiveView] = useState<"chat" | "editor" | "activeagents">("chat");
  ...
  const [terminalOutput, setTerminalOutput] = useState("BFD v0.15.0\nReady.\n");
  ...
  function buildSystemPrompt(modesOverride?: Modes, agentDocsOverride?: string): string {
    ...
    if (m.interaction === "agent") {
      prompt += "\n\n" + AGENT_MODE_PROMPT;
      prompt += docs || `\n\n(No agent definition files were found in ${AGENT_DIRS.join(" or ")}.)`;
    }
  ...
  // Switching to Chat (or re-picking while already in Agent) just flips the
  // mode; entering Agent mode fresh goes through the category picker instead
  // (see openAgentPicker / selectAgentCategory below).
  function selectInteraction(mode: InteractionMode) {
    if (isStreaming || mode === modes.interaction) return;
    setModes({ ...modes, interaction: mode });
  }

  // Clicking "Agent" always opens the enumerated picker first — which project
  // or agent group to discuss, with "New" as the last option — rather than
  // jumping straight into a generic kickoff.
  async function openAgentPicker() {
    if (isStreaming) return;
    try {
      const md = await invoke<string>("read_file", { path: AGENTS_REGISTRY_PATH });
      setAgentCategories(parseAgentRegistry(md));
    } catch {
      setAgentCategories([]);
    }
  ...
  // category === null means "New" was picked.
  async function selectAgentCategory(category: AgentCategory | null) {
    setShowAgentPicker(false);
    if (isStreaming) return;
    const newModes = { ...modes, interaction: "agent" as InteractionMode };
    setModes(newModes);

    // Load agent files into context, then have the assistant proactively
    // open the discussion, scoped to the picked category. The kickoff
    // instruction is sent as the Human turn but never shown in the transcript.
    setIsStreaming(true);
    ...
    const newApiHistory = await chatLoop([...messages], historyForModel(newModes.memory), {
      injectedUserText: category ? AGENT_KICKOFF_PROMPT(category) : NEW_AGENT_KICKOFF_PROMPT,
  ...
  async function openStartupTabs() {
    ...
    try {
      const parsed = JSON.parse(await invoke<string>("read_file", { path: STARTUP_TABS_CONFIG }));
    ...
    for (const spec of specs) {
      try {
        if (spec.path) {
          targets.push({ name: spec.path.split("/").pop() || spec.path, path: spec.path });
        } else if (spec.dir) {
          ...
          for (const e of picked) {
            targets.push(spec.file ? { name: e.name, path: `${e.path}/${spec.file}` } : { name: e.name, path: e.path });
          }
        }
    ...
    for (const t of targets) {
      ...
      try {
        ...
        opened.push({ name: t.name, path: t.path, content, language: detectLanguage(t.path.split("/").pop() || t.name), modified: false });
    ...
    <div className="app-container">
      ...
      <div className="mode-bar">
        ...
        <ModeGroup
          ...
          onSelect={(k) => { if (k === "agent") openAgentPicker(); else selectInteraction(k as InteractionMode); }}
          options={[
      ...
      {showAgentPicker && (
        <div className="agent-picker-overlay" onClick={() => setShowAgentPicker(false)}>
          <div className="agent-picker" onClick={(e) => e.stopPropagation()}>
            <div className="agent-picker-header">Agent mode — choose a project or agent group</div>
            {agentCategories.length === 0 ? (
              <div className="agent-picker-empty">No categories found in AGENTS.md.</div>
            ) : (
              agentCategories.map((c, i) => (
                <button key={i} className="agent-picker-item" onClick={() => selectAgentCategory(c)}>
                  <span className="agent-picker-kind">{c.kind}</span>
                  <span className="agent-picker-name">{c.name}</span>
                </button>
              ))
            )}
            <button className="agent-picker-item agent-picker-new" onClick={() => selectAgentCategory(null)}>
              <span className="agent-picker-kind">+</span>
              <span className="agent-picker-name">New</span>
            </button>
          </div>
        </div>
      )}
      ...
      <div className="main-content">
        <div className="sidebar">
          ...
          <div className="file-tree">
            ...
            <div className={`file-item ${activeView === "activeagents" ? "active" : ""}`} onClick={() => setActiveView("activeagents")}>
              🤖 Active Agents
            </div>
            <div className="sidebar-header" style={{ marginTop: 4, display: "flex", alignItems: "center" }}>
              <span style={{ cursor: "pointer", marginRight: 4 }} onClick={() => { const parent = currentDir.split("/").slice(0, -1).join("/"); if (parent) loadDir(parent); }} title="Go up">⬆️</span>
        ...
        <div className="center-panel">
          <div className="tabs-bar">
            ...
            <div className={`tab ${activeView === "activeagents" ? "active" : ""}`} onClick={() => setActiveView("activeagents")}>🤖 Active Agents</div>
            {tabs.map((tab, i) => (
      ...
      <div className="status-bar">
        ...
        <span>
          ...
          {" "}
          {activeView === "editor" && tabs[activeTab] ? `${tabs[activeTab].language}${tabs[activeTab].modified ? " (modified)" : ""}` : activeView === "activeagents" ? "active agents" : "chat"}
        </span>
      </div>
    </div>
  );
